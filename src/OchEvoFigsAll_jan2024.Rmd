...
Grazing new, using alpha renamed "attack" in main
graz refers to graz.c.day in supplement
...
resp.100$pgC.cell <- cn_hold100$pgC.cell[match(resp.100$Lineage,cn_hold100$Lineage)]
resp.100$resp.cue <- resp.100$pgC.cell * resp.100$Resp
...
cue.resp.avg.100 <- summarySE(data = resp.100, measurevar = 'resp.cue', groupvars = c('Strain', 'Ev', 'Acc', 'Rep'))
cue.resp.avg.100$ID <- paste(cue.resp.avg.100$Strain, cue.resp.avg.100$Ev, cue.resp.avg.100$Acc, cue.resp.avg.100$Rep, sep = ".")

cue.gr.avg.100$Resp.avg <- cue.resp.avg.100$resp.cue[match(cue.gr.avg.100$ID,cue.resp.avg.100$ID)]
...
resp.50$pgC.cell <- cn_hold50$pgC.cell[match(resp.50$Lineage,cn_hold50$Lineage)]
resp.50$resp.cue <- resp.50$pgC.cell * resp.50$Resp
...
cue.resp.avg.50 <- summarySE(data = resp.50, measurevar = 'resp.cue', groupvars = c('Strain', 'Ev', 'Acc', 'Rep'))
cue.resp.avg.50$ID <- paste(cue.resp.avg.50$Strain, cue.resp.avg.50$Ev, cue.resp.avg.50$Acc, cue.resp.avg.50$Rep, sep = ".")


cue.gr.avg.50$Resp.avg <- cue.resp.avg.50$resp.cue[match(cue.gr.avg.50$ID,cue.resp.avg.50$ID)]
...
ylim4<- resp.y <- c(0,3.1)
ylim7<- fvfm.y <-c(.35,.6)
#ylim8 <-n.y <-c()
ylim9 <-fsc.y<-c(6000, 9350)
...
ggarrange(a1, c1, b1,  d1,
          ...
          labels = c("(a)", "(b)", "(c)","(d)",
                     ...
                     "(q)","(r)","(s)","(t)",
                     "(u)","(v)","(w)","(x)"),
          font.label = list(size = 10, color = "black", family = NULL, face = "bold"),
          label.x = 0,
          label.y = 1,
...
#grazing
  ...
  geom_point(aes(color = as.factor(Ev)), alpha = .5, size = 1.5) +
   ...
   #attack rate
   ggplot(graz, aes(x = Assay, y = g.alpha.day )) +
  geom_point(aes(color = as.factor(Ev)), alpha = .5, size = 1.5) +
               scale_color_manual(values = newcols)+
   #coord_cartesian(ylim=c(-8, 50))+
    scale_y_continuous(limits = c(-2,15))+
   geom_smooth(aes(x = Assay, y = g.alpha.day, color = as.factor(Ev),fill = as.factor(Ev)),alpha = .4, method = "loess", span = .75, size = .5) +
     scale_fill_manual(values = newcols) +
     geom_hline(yintercept = 0)+
     theme(legend.position = "none",
        axis.text.y=element_text(size = 11),
      ...
      ggplot(graz, aes(x = Assay, y = g.alpha.day.2 )) +
  geom_point(aes(color = as.factor(Ev)), alpha = .5, size = 1.5) +
               scale_color_manual(values = newcols)+
   #coord_cartesian(ylim=c(-8, 50))+
  scale_y_continuous(limits = c(-2,20))+
   geom_smooth(aes(x = Assay, y = g.alpha.day.2, color = as.factor(Ev),fill = as.factor(Ev)),alpha = .4, method = "loess", span = .75, size = .5) +
     scale_fill_manual(values = newcols) +
     geom_hline(yintercept = 0)+
     theme(legend.position = "none",
        axis.text.y=element_text(size = 11),
...
theme_set(theme_base(base_size = 10)) 
theme_update(plot.background=element_blank())
figs2<- ggplot(qaqc, aes(x = RedF, y = ChlperCell, fill = as.factor(Strain)))+
...
theme(legend.position = "NONE",
       axis.ticks.length = unit(.10, "cm"),
      axis.text.y = element_text(size = labelcex),
       axis.text.x = element_text(size = labelcex),
      axis.title.y = element_text(size = labelcex),
       axis.title.x = element_text(size = labelcex)) +
labs( y ="Chlorophyll per cell", x="Red fluorescence") +
   geom_text(x = 3000, y = .5, label = lm_eqn91(lm.91), parse = TRUE, color = "gray50", hjust = "left", size = 4)+
  geom_text(x = 3000, y = .45, label = lm_eqn51(lm.51), parse = TRUE, hjust= "left", size =4)
figs2

#ggsave("test.png",figs2, dpi=300)
