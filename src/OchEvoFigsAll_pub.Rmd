...
pvals.df.100.2 <- as.data.frame(unique(ttest.df.100$group))
...
for(i in 1:dim(pvals.df.100.2)[1]){
  ...
  hold.ttest <- t.test(hold)
  tidy.hold <- tidy(hold.ttest)
  pvals.df.100.2$p.value.gr[i] <- round(tidy.hold$p.value,3) }
# REDF
for(i in 1:dim(pvals.df.100.2)[1]){
  hold <-ttest.df.100.2[ttest.df.100.2$group==pvals.df.100.2$group[i],] %>% 
    pull(redf.del)
...
# ## CUE
# ttest.df.100.cue <- cue.gr.avg.100 %>% 
#   filter(Strain != 1391) %>% 
#   select( ID, Strain, Ev, Acc, Rep, Week, Grazing.del) %>% 
#   filter(Ev==Acc) %>% 
#   filter(Ev != 24) %>% 
...
#   
# pvals.df.100.2 <- pvals.df.100.2 %>% 
#   mutate(signif.alpha = case_when(
#     pvals.df.100.2$p.value <= .001 ~ 1,
#     pvals.df.100.2$p.value >.001 & pvals.df.100.2$p.value <= .01 ~ .75,
#     pvals.df.100.2$p.value > .01 & pvals.df.100.2$p.value <= .05 ~ .5,
#     pvals.df.100.2$p.value > .05 ~ 0
#   ))
...
# N
a8<-a3<-ggplot(subset(N.summ.100.2, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=N, color=as.factor(Ev))) + 
...
# N
b8<-a3<-ggplot(subset(N.summ.100.2, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=N, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=N-se, ymax=N+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
...
# N
c8<-a3<-ggplot(subset(N.summ.100.2, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=N, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=N-se, ymax=N+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
...
# N
d8<-a3<-ggplot(subset(N.summ.100.2, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=N, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=N-se, ymax=N+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
