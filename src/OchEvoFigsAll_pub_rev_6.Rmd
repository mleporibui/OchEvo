...
dat100<-read_csv(here::here("data","OECD100_rev3.csv"))
dat50<-read_csv(here::here("data","OECD50_rev3.csv"))
...
dat100<- dat100 %>% 
  mutate(redF.corr = case_when(
    Strain == "1391" ~ (2.732e-05)*(Mean.redF) + (6.702e-03) ,
  ...
  #Edit: use alpha for attack rate, grazing rate to supplement
# grazing: convert bact per och per day to pgC per och per day 
...
dat100$attack.day <- dat100$graz.alpha *24 *10^5
dat50$attack.day <- dat50$graz.alpha *24 *10^5
...
Calculate generations elapsed per treatment based on growth rate and experimental time
...
dat100$doubtime <- 1.520030687
dat100<- dat100 %>%
 mutate(doubtime = case_when(
    Strain == 1391 & Ev == 18|Strain == 1391 & Ev == 24 & Acc ==18  ~ 3.040061373,
      TRUE ~ doubtime))
    
dat100$days <- dat100$Week * 7
dat100$Generations <- dat100$days / dat100$doubtime

dat50$doubtime <- 1.520030687
dat50<- dat50 %>%
 mutate(doubtime = case_when(
    Strain == 1391 & Ev == 18|Strain == 1391 & Ev == 24 & Acc ==18  ~ 3.040061373,
      TRUE ~ doubtime))
...
dat50$days <- dat50$Week * 7
dat50$Generations <- dat50$days / dat50$doubtime
...
evol.grs.100<-list(gr.1391.18.100=gr.1391.18.100,
               gr.1391.24.100=gr.1391.24.100,
               gr.1391.30.100=gr.1391.30.100,
...
evol.grs.50<-list(gr.1391.18.50=gr.1391.18.50,
               gr.1391.24.50=gr.1391.24.50,
               gr.1391.30.50=gr.1391.30.50,
...
dat100<- dat100 %>%
 mutate(gens.elaps = case_when(
    Strain == 1391 & Ev == 18|Strain == 1391 & Ev == 24 & Acc ==18  ~ sr.100.1391.18.18$gens.elaps[match(dat100$Days,sr.100.1391.18.18$x)],
    Strain == 2951 & Ev == 18|Strain == 2951 & Ev == 24 & Acc ==18  ~ sr.100.2951.18.18$gens.elaps[match(dat100$Days,sr.100.2951.18.18$x)],
    Strain == 1391 & Ev == 30|Strain == 1391 & Ev == 24 & Acc ==30  ~ sr.100.1391.30.30$gens.elaps[match(dat100$Days,sr.100.1391.30.30$x)],
    Strain == 2951 & Ev == 30|Strain == 2951 & Ev == 24 & Acc ==30  ~ sr.100.2951.30.30$gens.elaps[match(dat100$Days,sr.100.2951.30.30$x)],
    Strain == 1391 & Ev == 24 & Acc == 24 ~ sr.100.1391.24.24$gens.elaps[match(dat100$Days,sr.100.1391.24.24$x)],
    Strain == 2951 & Ev == 24 & Acc == 24 ~ sr.100.2951.24.24$gens.elaps[match(dat100$Days,sr.100.2951.24.24$x)]
    ))

dat50<- dat50 %>%
 mutate(gens.elaps = case_when(
    Strain == 1391 & Ev == 18|Strain == 1391 & Ev == 24 & Acc ==18  ~ sr.50.1391.18.18$gens.elaps[match(dat50$Days,sr.50.1391.18.18$x)],
    Strain == 2951 & Ev == 18|Strain == 2951 & Ev == 24 & Acc ==18  ~ sr.50.2951.18.18$gens.elaps[match(dat50$Days,sr.50.2951.18.18$x)],
    Strain == 1391 & Ev == 30|Strain == 1391 & Ev == 24 & Acc ==30  ~ sr.50.1391.30.30$gens.elaps[match(dat50$Days,sr.50.1391.30.30$x)],
    Strain == 2951 & Ev == 30|Strain == 2951 & Ev == 24 & Acc ==30  ~ sr.50.2951.30.30$gens.elaps[match(dat50$Days,sr.50.2951.30.30$x)],
    Strain == 1391 & Ev == 24 & Acc == 24 ~ sr.50.1391.24.24$gens.elaps[match(dat50$Days,sr.50.1391.24.24$x)],
...
### rev
dat100$gens.elaps <- dat100$Generations
### rev
dat50$gens.elaps <- dat50$Generations
...
#add fsc sept2022
growths100$fsc <- growth_hold100$Mean.FSC[match(growths100$ID,growth_hold100$ID)]
growths100$fsc.ref <- growth_hold100[growth_hold100$Ev==24,]$Mean.FSC[match(growths100$Treat.Acc,growth_hold100[growth_hold100$Ev==24,]$Treat.Acc)] 
growths100$fsc.propdel <- (growths100$fsc-growths100$fsc.ref)/abs(growths100$fsc.ref) # Compute rel. difference
growths100$fsc.del <- (growths100$fsc-growths100$fsc.ref) # Compute abs difference


growths100 <- growths100[!is.na(growths100$Growth.Rate),]
growths100<- growths100[growths100$Strain != 1393,]
...
#add fsc sept2022
growths50$fsc <- growth_hold50$Mean.FSC[match(growths50$ID,growth_hold50$ID)]
growths50$fsc.ref <- growth_hold50[growth_hold50$Ev==24,]$Mean.FSC[match(growths50$Treat.Acc,growth_hold50[growth_hold50$Ev==24,]$Treat.Acc)] 
growths50$fsc.propdel <- (growths50$fsc-growths50$fsc.ref)/abs(growths50$fsc.ref) # Compute rel. difference
growths50$fsc.del <- (growths50$fsc-growths50$fsc.ref) # Compute abs difference
...
Grazing using alpha / attack rate in main text

```{r, echo = FALSE, warning = FALSE}
# dataframes for grazing, 

# 100uE 
# cn_hold100 <- dat100 %>% 
#   filter(Week == 141)
attack_hold100 <- dat100 %>% 
  filter( !is.na (attack.day))
attack_hold100$ID <- paste(attack_hold100$Strain,attack_hold100$Ev,attack_hold100$Acc,attack_hold100$Rep,attack_hold100$Week,sep='.')
...
attack_hold100$attack <- attack_hold100$attack.day #/ graz_hold100$C141
...
attack_hold50 <- dat50 %>% 
  filter( !is.na (attack.day))
attack_hold50$ID <- paste(attack_hold50$Strain,attack_hold50$Ev,attack_hold50$Acc,attack_hold50$Rep,attack_hold50$Week,sep='.')
# graz_hold50$C141 <- cn_hold50$pgC.cell[match(graz_hold50$Lineage,cn_hold50$Lineage)]
attack_hold50$attack <- attack_hold50$attack.day #/ graz_hold50$C141

# grazing data frame 
attack_hold100$Treat.Acc <- paste(attack_hold100$Strain,attack_hold100$Acc,attack_hold100$Rep,attack_hold100$Week,sep='.')
attack.100 <- as.data.frame(unique(attack_hold100$ID))
names(attack.100) <- 'ID'
attack.100$Treat.Acc <- attack_hold100$Treat.Acc[match(attack.100$ID,attack_hold100$ID)]
attack.100$Lineage <- attack_hold100$Lineage[match(attack.100$ID,attack_hold100$ID)]
attack.100$Strain <- attack_hold100$Strain[match(attack.100$ID,attack_hold100$ID)]
attack.100$Ev <- attack_hold100$Ev[match(attack.100$ID,attack_hold100$ID)]
attack.100$Acc <- attack_hold100$Acc[match(attack.100$ID,attack_hold100$ID)]
attack.100$Rep <- attack_hold100$Rep[match(attack.100$ID,attack_hold100$ID)]
attack.100$Week <- attack_hold100$Week[match(attack.100$ID,attack_hold100$ID)]
attack.100$gens.elaps <- attack_hold100$gens.elaps[match(attack.100$ID,attack_hold100$ID)]
attack.100$attack <- attack_hold100$attack[match(attack.100$ID,attack_hold100$ID)]
attack.100$attack.ref <- attack_hold100[attack_hold100$Ev==24,]$attack[match(attack.100$Treat.Acc,attack_hold100[attack_hold100$Ev==24,]$Treat.Acc)] 
attack.100 <- attack.100[!is.na(attack.100$attack),]

# Match evolved & ancestral reference points
attack.100$Attack.propdel <- (attack.100$attack-attack.100$attack.ref)/abs(attack.100$attack.ref) # Compute rel. difference
attack.100$Attack.del <- (attack.100$attack-attack.100$attack.ref) # Compute abs difference

# repeat for 50uE
attack_hold50$Treat.Acc <- paste(attack_hold50$Strain,attack_hold50$Acc,attack_hold50$Rep,attack_hold50$Week,sep='.')
attack.50 <- as.data.frame(unique(attack_hold50$ID))
names(attack.50) <- 'ID'
attack.50$Treat.Acc <- attack_hold50$Treat.Acc[match(attack.50$ID,attack_hold50$ID)]
attack.50$Lineage <- attack_hold50$Lineage[match(attack.50$ID,attack_hold50$ID)]
attack.50$Strain <- attack_hold50$Strain[match(attack.50$ID,attack_hold50$ID)]
attack.50$Ev <- attack_hold50$Ev[match(attack.50$ID,attack_hold50$ID)]
attack.50$Acc <- attack_hold50$Acc[match(attack.50$ID,attack_hold50$ID)]
attack.50$Rep <- attack_hold50$Rep[match(attack.50$ID,attack_hold50$ID)]
attack.50$Week <- attack_hold50$Week[match(attack.50$ID,attack_hold50$ID)]
attack.50$gens.elaps <- attack_hold50$gens.elaps[match(attack.50$ID,attack_hold50$ID)]
attack.50$attack <- attack_hold50$attack[match(attack.50$ID,attack_hold50$ID)]
attack.50$attack.ref <- attack_hold50[attack_hold50$Ev==24,]$attack[match(attack.50$Treat.Acc,attack_hold50[attack_hold50$Ev==24,]$Treat.Acc)] 
attack.50 <- attack.50[!is.na(attack.50$attack),]

# Match evolved & ancestral reference points
attack.50$Attack.propdel <- (attack.50$attack-attack.50$attack.ref)/abs(attack.50$attack.ref) # Compute rel. difference
attack.50$Attack.del <- (attack.50$attack-attack.50$attack.ref) # Compute abs difference



```

Grazing in pgC per pgC per day == gC per gC per day (for supplement)
...
par(mar=c(5,6,0,0),mgp=c(3,2,0),mfrow=c(2,2),oma=c(4,4,0,0))
...
plot(del$gens.elaps,del$Growth.del,ylim=c(-.4,.8),type='n',ylab='',xlab='',xaxt = 'n', las=1,xlim=c(0,720),cex.lab=labelcex,cex.axis=labelcex,cex.main=maincex); 
...
text(-3,.7,'(A) Obligate, high light',cex=labelcex,pos=4)
# text(del$gens.elaps, .7,  del$signif,
...
del$signif.alpha <- pvals.df.100$signif.alpha[match(del$group,pvals.df.100$group)]
xtick <- seq(0,800, by = 100)
plot(del$gens.elaps,del$Growth.del,ylim=c(-.4,.8),type='n',ylab='',xlab='',xaxt = 'n', yaxt = 'n', las=1,xlim=c(0,720),cex.lab=labelcex,cex.axis=labelcex,cex.main=maincex); abline(h=0);
axis(side=1, at=xtick, labels = FALSE);
...
# text(del$gens.elaps, .7,  del$signif,
#      cex=5,col=hcol2,srt=-90, pos=1) 
text(-3,.7,'(C) Facultative, high light',cex=labelcex,pos=4)
...
arrows(del$gens.elaps,del$Growth.del+del$sd,del$gens.elaps,del$Growth.del-del$sd,code=3,length=.1,angle=90); lines(del$gens.elaps,del$Growth.del,col=hcol2,lwd=lwdset); points(del$gens.elaps,del$Growth.del,pch=21,col = hcol2,bg = "white", cex=ptcex, lwd = lwdset)
points(del$gens.elaps,del$Growth.del,pch=19,col = alpha(hcol2, alpha = del$signif.alpha),cex=ptcex)
text(-3,.7,'(B) Obligate, low light',cex=labelcex,pos=4)
# text(del$gens.elaps, .7,  del$signif,
#      cex=5,col=hcol2,srt=-90, pos=1) 
...
plot(del$gens.elaps,del$Growth.del,ylim=c(-.4,.8),type='n',ylab='',xlab='',yaxt = 'n',
      las=1,xlim=c(0,720),cex.lab=labelcex,cex.axis=labelcex,cex.main=maincex); 
axis(side=2, at=ytick, labels = FALSE);
...
points(del$gens.elaps,del$Growth.del,pch=19,col = alpha(hcol2, alpha = del$signif.alpha),cex=ptcex)
text(-3,.7,'(D) Facultative, low light',cex=labelcex,pos=4)
# text(del$gens.elaps, .7,  del$signif,
#      cex=5,col=hcol2,srt=-90, pos=1) 

legend(600,0.8, legend = c(lp1,lp2,lp3,lp4), pch= c(21,21,21,21),col = "black", pt.bg = alpha("black", alpha = c(1,.75,.5,0)), pt.cex = ptcex, pt.lwd = lwdset, cex = labelcex )


mtext("Generations",side=1,line=0,outer=TRUE,cex=labelcex)
mtext("Difference in growth rate d-1",side=2,line=0,outer=TRUE,cex=labelcex,las=0)
...
Figure 2. Thermal adaptation in obligate phototrophs. To test for evolutionary responses, we computed the difference in growth rates between experimental and acclimated control lineages at hot (red) and cold (blue) temperatures. Positive values are evidence of adaptive evolution (experimental lineages growing faster than acclimated control lineages assayed at the same temperature). The x-axes are scaled by growth rate to show time in number of generations the evolving strains experienced at their evolutionary temperatures. Data are shown for both mixotroph strains—the obligate phototroph Strain 1391 (left column) and the facultative phototroph Strain 2951 (right column) – and both light levels – high light (100 μmol quanta·m−2·s−1 top row) and low light (50 μmol quanta·m−2·s−1, bottom row) for each reciprocal transplant experiment. Point coloration indicates significance ranging from p<0.001 (darkest points; one sample Student’s t-test) to p>0.05 (white). 
...
fsc.summ.100 <- summarySE(data=growths100_last3,measurevar='fsc',groupvars=c('Strain','Ev','Acc'))
Del.fsc.summ.100 <- summarySE(data=growths100_last3,measurevar='fsc.del',groupvars=c('Strain','Ev','Acc'))
...
fsc.summ.50 <- summarySE(data=growths50_last3,measurevar='fsc',groupvars=c('Strain','Ev','Acc'))
Del.fsc.summ.50 <- summarySE(data=growths50_last3,measurevar='fsc.del',groupvars=c('Strain','Ev','Acc'))
...
# attack rate
attack100_last3<-attack.100 %>% 
  filter(Strain != 1393) %>% 
  filter(Week %in% c(118,130,142))
Attack.summ.100 <- summarySE(data=attack100_last3,measurevar='attack',groupvars=c('Strain','Ev','Acc'))
attack50_last3<-attack.50 %>% 
  filter(Strain != 1393) %>% 
  filter(Week %in% c(118,130,142))
Attack.summ.50 <- summarySE(data=attack50_last3,measurevar='attack',groupvars=c('Strain','Ev','Acc'))
...
ttest.df.100.3 <- growths100_last3 %>% 
  filter(Strain != 1393) %>% 
  select( ID, Strain, Ev, Acc, Rep, Week, Growth.Rate, Growth.Rate.ref,redf,redf.ref,fvfm,fvfm.ref,fsc,fsc.ref) %>% 
  filter(Ev==Acc) %>% 
  filter(Ev != 24)
...
for(i in 1:dim(pvals.df.100.3)[1]){
  hold1 <-ttest.df.100.3[ttest.df.100.3$group==pvals.df.100.3$group[i],] %>% 
   ...
   hold.ttest <- t.test(hold1, hold2)
  tidy.hold <- tidy(hold.ttest)
  pvals.df.100.3$p.value.fsc[i] <- round(tidy.hold$p.value,3) }
...
## Attack rate
ttest.df.100.attack <- attack100_last3 %>% 
  filter(Strain != 1393) %>% 
  select( ID, Strain, Ev, Acc, Rep, Week, attack, attack.ref) %>% 
  filter(Ev==Acc) %>% 
  filter(Ev != 24) %>% 
  filter(!is.na(attack.ref))
ttest.df.100.attack$group <- paste(ttest.df.100.attack$Strain,ttest.df.100.attack$Ev,ttest.df.100.attack$Acc,sep='.')
pvals.df.100.3$p.value.attack <- NA
# attack
for(i in 1:dim(pvals.df.100.3)[1]){
  hold1 <-ttest.df.100.attack[ttest.df.100.attack$group==pvals.df.100.3$group[i],] %>% 
    pull(attack)
  hold2 <-ttest.df.100.attack[ttest.df.100.attack$group==pvals.df.100.3$group[i],] %>% 
    pull(attack.ref)
  hold.ttest <- t.test(hold1, hold2)
  tidy.hold <- tidy(hold.ttest)
  pvals.df.100.3$p.value.attack[i] <- round(tidy.hold$p.value,3) }
...
pvals.df.100.3.sig[ ,5:14][pvals.df.100.3.sig [,5:14] > .05] <- NA
pvals.df.100.3.sig[ ,5:14][pvals.df.100.3.sig [,5:14] <=.001] <- "high"
pvals.df.100.3.sig[ ,5:14][pvals.df.100.3.sig [,5:14] <=.01] <-"mid"
pvals.df.100.3.sig[ ,5:14][pvals.df.100.3.sig [,5:14] <=.05] <-"low"

pvals.df.100.3.sig[ ,5:14][pvals.df.100.3.sig [,5:14] == "high"] <- "***"
pvals.df.100.3.sig[ ,5:14][pvals.df.100.3.sig [,5:14] =="mid"] <- "**"
pvals.df.100.3.sig[ ,5:14][pvals.df.100.3.sig [,5:14] =="low"] <-  "*"
...
for(i in 1:dim(pvals.df.50.3)[1]){
  hold1 <-ttest.df.50.3[ttest.df.50.3$group==pvals.df.50.3$group[i],] %>% 
   ...
   hold.ttest <- t.test(hold1, hold2)
  tidy.hold <- tidy(hold.ttest)
  pvals.df.50.3$p.value.fsc[i] <- round(tidy.hold$p.value,3) }
...
## Attack rate
ttest.df.50.attack <- attack50_last3 %>% 
  filter(Strain != 1393) %>% 
  select( ID, Strain, Ev, Acc, Rep, Week, attack, attack.ref) %>% 
  filter(Ev==Acc) %>% 
  filter(Ev != 24) %>% 
  filter(!is.na(attack.ref))
ttest.df.50.attack$group <- paste(ttest.df.50.attack$Strain,ttest.df.50.attack$Ev,ttest.df.50.attack$Acc,sep='.')
pvals.df.50.3$p.value.attack <- NA
# attack
for(i in 1:dim(pvals.df.50.3)[1]){
  hold1 <-ttest.df.50.attack[ttest.df.50.attack$group==pvals.df.50.3$group[i],] %>% 
    pull(attack)
  hold2 <-ttest.df.50.attack[ttest.df.50.attack$group==pvals.df.50.3$group[i],] %>% 
    pull(attack.ref)
  hold.ttest <- t.test(hold1, hold2)
  tidy.hold <- tidy(hold.ttest)
  pvals.df.50.3$p.value.attack[i] <- round(tidy.hold$p.value,3) }
...
pvals.df.50.3.sig[ ,5:14][pvals.df.50.3.sig [,5:14] > .05] <- NA
pvals.df.50.3.sig[ ,5:14][pvals.df.50.3.sig [,5:14] <=.001] <- "high"
pvals.df.50.3.sig[ ,5:14][pvals.df.50.3.sig [,5:14] <=.01] <-"mid"
pvals.df.50.3.sig[ ,5:14][pvals.df.50.3.sig [,5:14] <=.05] <-"low"

pvals.df.50.3.sig[ ,5:14][pvals.df.50.3.sig [,5:14] == "high"] <- "***"
pvals.df.50.3.sig[ ,5:14][pvals.df.50.3.sig [,5:14] =="mid"] <- "**"
pvals.df.50.3.sig[ ,5:14][pvals.df.50.3.sig [,5:14] =="low"] <-  "*"
...
#limits
ylim1 <- growth.y <- c(0,.8)
ylim0 <- CUE.y <- c(0, 1)
ylim3 <- C.y <- c(10,30)
ylim2 <- redf.y <-c(8,22)
ylim5 <-phot.y <- c(0,1.25)
ylim6<- graz.y <- c(0,.75)
ylim6a<- graz.a.y <-c(1,13)
ylim4<- resp.y <- c(0,3)
ylim7<- fvfm.y <-c(.35,.56)
#ylim8 <-n.y <-c()
ylim9 <-fsc.y<-c(6000, 9250)

labelcex <- 10
sigsize <- 5

#Growth rate of 1391 at 100 by acclimation temperature with experimental lineages in blue and red and control lineages in gray
...
#  axis.text.x = element_blank(),       
  ...
  labs( y = "Carbon use efficiency", x=NULL) +
...
theme(legend.position = "none",
    ...
    geom_line(color = "black") +
    geom_point(size=3, aes()) +
  labs( y = "Chlorophyll mgChl-a gC-1", x=NULL) +
...
theme(legend.position = "none",
    ...
    geom_line(color = "black") +
    geom_point(size=3, aes()) +
labs( y = "Cellular C pgC cell-1", x=NULL) +
...
a4 <- ggplot(subset(Resp.summ.100, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=Resp, color=as.factor(Ev))) + 
    ...
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
                     expand = c(0,0)) +
  scale_y_continuous(limits = ylim4)+
theme(legend.position = "none",
      axis.title.x=element_blank(),
      axis.text.x=element_blank(),       
      axis.text.y=element_text(size = labelcex),
    ...
    geom_line(color = "black") +
    geom_point(size=3, aes()) +
 labs( y = "Respiration gC gC-1 d-1", x=NULL) +
...
a5 <- ggplot(subset(Photo.summ.100, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=Photo, color=as.factor(Ev))) + 
    ...
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
                     expand = c(0,0)) +
  scale_y_continuous(limits = ylim5)+
 theme(legend.position = "none",
      axis.title.x=element_blank(),
      axis.text.x=element_blank(),       
      axis.text.y=element_text(size = labelcex),
    ...
    geom_line(color = "black") +
    geom_point(size=3, aes()) +
  labs( y = "Photosynthesis gC gC-1 d-1", x=NULL) +
...
a6 <- ggplot(subset(Graz.summ.100, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=graz, color=as.factor(Ev))) + 
    ...
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
                     expand = c(0,0)) +
  scale_y_continuous(limits = ylim6)+
  theme(legend.position = "none",
      axis.title.x=element_blank(),
      axis.text.x=element_blank(),       
      axis.text.y=element_text(size = labelcex),
    ...
    geom_line(color = "black") +
    geom_point(size=3, aes()) +
 labs( y = "Grazing gC gC-1 d-1", x=NULL) +
...
# Attack rate
a6a <- ggplot(subset(Attack.summ.100, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=attack, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=attack-se, ymax=attack+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
                     expand = c(0,0)) +
  scale_y_continuous(limits = ylim6a)+
  theme(legend.position = "none",
      axis.title.x=element_blank(),
      axis.text.x=element_blank(),       
      axis.text.y=element_text(size = labelcex),
    ...
    geom_line(color = "black") +
    geom_point(size=3, aes()) +
 labs( y = "Attack rate d-1 (x10^5)", x=NULL) +
  geom_errorbar(data = subset(Attack.summ.100, Ev==24 & Strain == strainchoice),
                aes(ymin=attack-se, ymax=attack+se, width = .2), color = "gray48") +
  geom_line(data = subset(Attack.summ.100, Ev==24 &Strain == strainchoice), color = "gray48", linetype = "dashed") +
  geom_point(data = subset(Attack.summ.100, Ev==24 & Strain == strainchoice), color = "gray48", aes())+
  annotate("text", x= 21,  y = Inf, vjust = 1.5, label = points.sig[points.sig$Ev==18,]$p.value.attack, color = lcol2, size = sigsize) +
  annotate("text", x= 27,  y = Inf, vjust = 1.5, label = points.sig[points.sig$Ev==30,]$p.value.attack, color = hcol2, size = sigsize)
...
a7<-ggplot(subset(fvfm.summ.100, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=fvfm, color=as.factor(Ev))) + 
    ...
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
                     expand = c(0,0)) +
   scale_y_continuous(limits = ylim7)+
theme(legend.position = "none",
      axis.title.x=element_blank(),
      axis.text.x=element_blank(),       
      axis.text.y=element_text(size = labelcex),
...
a9<-ggplot(subset(fsc.summ.100, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=fsc, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=fsc-se, ymax=fsc+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
                     expand = c(0,0)) +
      scale_y_continuous(limits = ylim9)+
theme(legend.position = "none",
      axis.title.x=element_blank(),
  #    axis.text.x=element_blank(),       
      axis.text.y=element_text(size = labelcex),
    ...
    geom_line(color = "black") +
    geom_point(size=3, aes()) +
labs( y = "Forward scatter", x=NULL) +
  geom_errorbar(data = subset(fsc.summ.100, Ev==24 & Strain == strainchoice),aes(ymin=fsc-se, ymax=fsc+se, width = .2), color = "gray48") +
  geom_line(data = subset(fsc.summ.100, Ev==24 &Strain == strainchoice), color = "gray48", linetype = "dashed") +
  geom_point(data = subset(fsc.summ.100, Ev==24 & Strain == strainchoice), color = "gray48", aes()) +
  annotate("text", x= 21,  y = Inf, vjust = 1.5, label = points.sig[points.sig$Ev==18,]$p.value.fsc, color = lcol2, size = sigsize) +
  annotate("text", x= 27,  y = Inf, vjust = 1.5, label = points.sig[points.sig$Ev==30,]$p.value.fsc, color = hcol2, size = sigsize) 
...
# Attack rate
b6a <- ggplot(subset(Attack.summ.100, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=attack, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=attack-se, ymax=attack+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
...
#fsc
b9<- ggplot(subset(fsc.summ.100, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=fsc, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=fsc-se, ymax=fsc+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
...
#Growth rate of 1391 at 50 by acclimation temperature with experimental lineages in blue and red and control lineages in gray
strainchoice <- 1391
lightchoice <- 50
...
# Attack rate
c6a <- ggplot(subset(Attack.summ.50, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=attack, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=attack-se, ymax=attack+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
  ...
  theme(legend.position = "none",
    ...
    geom_point(size=3, aes()) +
 labs( y = NULL, x=NULL) +
  geom_errorbar(data = subset(Attack.summ.50, Ev==24 & Strain == strainchoice),
                aes(ymin=attack-se, ymax=attack+se, width = .2), color = "gray48") +
  geom_line(data = subset(Attack.summ.50, Ev==24 &Strain == strainchoice), color = "gray48", linetype = "dashed") +
  geom_point(data = subset(Attack.summ.50, Ev==24 & Strain == strainchoice), color = "gray48", aes())+
  annotate("text", x= 21,  y = Inf, vjust = 1.5, label = points.sig[points.sig$Ev==18,]$p.value.attack, color = lcol2, size = sigsize) +
  annotate("text", x= 27,  y = Inf, vjust = 1.5, label = points.sig[points.sig$Ev==30,]$p.value.attack, color = hcol2, size = sigsize)
...
c9<-ggplot(subset(fsc.summ.50, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=fsc, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=fsc-se, ymax=fsc+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
...
theme(legend.position = "none",
    ...
    geom_point(size=3, aes()) +
labs( y = NULL, x=NULL) +
  geom_errorbar(data = subset(fsc.summ.50, Ev==24 & Strain == strainchoice),aes(ymin=fsc-se, ymax=fsc+se, width = .2), color = "gray48") +
  geom_line(data = subset(fsc.summ.50, Ev==24 &Strain == strainchoice), color = "gray48", linetype = "dashed") +
  geom_point(data = subset(fsc.summ.50, Ev==24 & Strain == strainchoice), color = "gray48", aes()) +
  annotate("text", x= 21,  y = Inf, vjust = 1.5, label = points.sig[points.sig$Ev==18,]$p.value.fsc, color = lcol2, size = sigsize) +
  annotate("text", x= 27,  y = Inf, vjust = 1.5, label = points.sig[points.sig$Ev==30,]$p.value.fsc, color = hcol2, size = sigsize) 
...
# Attack rate
d6a <- ggplot(subset(Attack.summ.50, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=attack, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=attack-se, ymax=attack+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
...
#FSC
d9<-ggplot(subset(fsc.summ.50, Ev==Acc & Strain == strainchoice), aes(x=Acc, y=fsc, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=fsc-se, ymax=fsc+se), width=.2) +
    scale_color_manual(values=newcols) +
    scale_x_continuous(limits = c(17,31),
                     breaks = c(18,24,30),
...
annotate_figure(fig3, bottom = ("Temperature C"))


# Figure S4
figs4<- ggarrange(a2, c2, b2, d2,
                  a7, c7, b7, d7,
                  a6, c6, b6, d6,
                  a9, c9, b9, d9,
       labels = c("A", "B", "C", "D",
   ...
   font.label = list(size = 12, color = "black", family = NULL),
          ...
          label.y = 1,
          ncol = 4, nrow = 6, align="hv" )
annotate_figure(figs4, bottom = ("Temperature C"))


```

Figure 3. Thermal responses of carbon cycle-relevant traits in mixotrophs. We measured the plastic response of control lineages acclimated to all temperatures (gray points, dotted lines) and the evolved response of experimental lineages at their treatment temperatures (cold-evolved in blue, hot-evolved in red, solid black line). Data for the obligate (Strain 1391, first and second column) and the facultative phototroph (Strain 2951, third and fourth column) are shown for both light levels (high light in first and third column, low light in second and fourth column). Points represent means for all 6 replicates across the final year of the project (2-3 time points) of the experiment, with error bars showing ± 1 standard error. We measured growth rate (A-D), cellular carbon content (E-H), photosynthesis (I-L), attack rate (M-P), respiration (Q-T), and carbon use efficiency (U-X). Significant differences between the evolved and acclimated response at treatment temperatures are shown at the top of each panel in blue for 18°C and red for 30°C (two-sample Student’s t-test; *** = p<0.001, ** = p<0.01, and * = p<0.05).


### 1391 at 100 bar graphs of deltas
...
ttest.df.100.attack <- attack100_last3 %>% 
  filter(Strain != 1393) %>% 
  select( ID, Strain, Ev, Acc, Rep, Week, Attack.del) %>% 
  filter(Ev==Acc) %>% 
  filter(Ev != 24) %>% 
  filter(!is.na(Attack.del))
ttest.df.100.attack$group <- paste(ttest.df.100.attack$Strain,ttest.df.100.attack$Ev,ttest.df.100.attack$Acc,sep='.')
pvals.df.100.2$p.value.attack <- NA

# attack
for(i in 1:dim(pvals.df.100.2)[1]){
  hold <-ttest.df.100.attack[ttest.df.100.attack$group==pvals.df.100.2$group[i],] %>% 
    pull(Attack.del)
  hold.ttest <- t.test(hold)
  tidy.hold <- tidy(hold.ttest)
  pvals.df.100.2$p.value.attack[i] <- round(tidy.hold$p.value,3) }
...
growths100_last3.91<-growths100_last3 %>% 
  filter(Strain == 1391) %>% 
  filter(Ev != 24) %>% 
  filter(Acc != 24) %>% 
  filter(Ev == Acc) 
growths100_last3.91$group <- paste(growths100_last3.91$Strain, growths100_last3.91$Ev,sep=".")
...
cn.100.91<-cn.100 %>% 
  ...
  filter(Acc != 24) %>% 
  filter(Ev == Acc) 
cn.100.91$group <- paste(cn.100.91$Strain, cn.100.91$Ev,sep=".")

redf.100_last3.91<-growths100_last3.91 %>% 
  filter(Strain == 1391) %>% 
  filter(Ev != 24) %>% 
  filter(Acc != 24) %>% 
  filter(Ev == Acc) 
redf.100_last3.91$group <- paste(redf.100_last3.91$Strain, redf.100_last3.91$Ev,sep=".")

photopt.100_last3.91<-resp.100_last3%>% 
  filter(Strain == 1391) %>% 
  filter(Ev != 24) %>% 
  filter(Acc != 24) %>% 
  filter(Ev == Acc) %>% 
  filter(!is.na(Photo.del))
photopt.100_last3.91$group <- paste(photopt.100_last3.91$Strain, photopt.100_last3.91$Ev,sep=".")

attack100_last3.91<-attack100_last3%>% 
  filter(Strain == 1391) %>% 
  filter(Ev != 24) %>% 
  filter(Acc != 24) %>% 
  filter(Ev == Acc) 
attack100_last3.91$group <- paste(attack100_last3.91$Strain, attack100_last3.91$Ev,sep=".")

resp.100_last3.91<-resp.100_last3%>% 
  filter(Strain == 1391) %>% 
  filter(Ev != 24) %>% 
  filter(Acc != 24) %>% 
  filter(Ev == Acc) %>% 
  filter(!is.na(Resp.del))
resp.100_last3.91$group <- paste(resp.100_last3.91$Strain, resp.100_last3.91$Ev,sep=".")

growths.91del <- summarySE(data=growths100_last3.91,measurevar='Growth.del',groupvars=c('Strain','Ev','Acc'))
c.91del <- summarySE(data=cn.100.91,measurevar='C.del',groupvars=c('Strain','Ev','Acc'))
redf.91del <- summarySE(data=redf.100_last3.91,measurevar='redf.del',groupvars=c('Strain','Ev','Acc'))
photopt.91del <- summarySE(data=photopt.100_last3.91,measurevar='Photo.del',groupvars=c('Strain','Ev','Acc'))
attack.91del <- summarySE(data=attack100_last3.91,measurevar='Attack.del',groupvars=c('Strain','Ev','Acc'))
resp.91del <- summarySE(data=resp.100_last3.91,measurevar='Resp.del',groupvars=c('Strain','Ev','Acc'))

#pvalues
bar.pvals <- pvals.df.100.2 %>% 
  filter(Strain == "1391") %>% 
  select( "group", "Ev", "p.value.growth", "p.value.redf","p.value.c", "p.value.resp", "p.value.photo", "p.value.attack")
...
p1<-  ggplot(growths.91del, aes(x=as.factor(Ev), y=Growth.del, fill=as.factor(Ev))) + 
...
p2<-  ggplot(c.91del, aes(x=as.factor(Ev), y=C.del, fill=as.factor(Ev))) + 
  geom_bar(stat="identity")  +
...
p3<-ggplot(redf.91del, aes(x=as.factor(Ev), y=redf.del, fill=as.factor(Ev))) + 
  geom_bar(stat="identity")  +
...
p4<-ggplot(photopt.91del, aes(x=as.factor(Ev), y=Photo.del, fill=as.factor(Ev))) + 
...
p5<-ggplot(attack.91del, aes(x=as.factor(Ev), y=Attack.del, fill=as.factor(Ev))) + 
  geom_bar(stat="identity")  +
  geom_errorbar(aes(ymin=Attack.del-se, ymax=Attack.del+se), width=.2) +
  scale_fill_manual(values=coldhot) +
   theme(legend.position = "none",
       # axis.text.x = element_blank(), 
        axis.text.y=element_text(size = labelcex),
        ...
        plot.margin = margin(.1, .1, .1, 1.5, "cm")) +
 labs( y = "Attack rate d-1 (x10^5)", x=NULL)  +
  annotate("text", x= 1,  y = .9, label = bar.pvals.sig[bar.pvals.sig$Ev=="18",]$p.value.attack) +
  annotate("text", x= 2,  y = .9, label = bar.pvals.sig[bar.pvals.sig$Ev=="30",]$p.value.attack)

p6<-ggplot(resp.91del, aes(x=as.factor(Ev), y=Resp.del, fill=as.factor(Ev))) + 
...
fig4<- ggarrange(p1,p2,p3,
          p4,p5,p6,
         labels = c("A", "B", "C",
                    "D", "E", "F"),
          nrow = 2, ncol = 3, align = "hv") 
annotate_figure(fig4, bottom = "Temperature C")

```

Figure 4. Differences between evolved and plastic responses of obligate phototroph at high light. Differences between evolved responses of the cold and warm evolved lineages and the plastic responses of the acclimated controls at the respective cold and warm temperatures (obtained from reciprocal transplant) are shown for several key traits, to examine what mechanisms drove adaptation in the obligate phototroph (Strain 1391) at high light (100 μmol quanta·m−2·s−1). The differences in cold-evolved lineages and acclimated controls are shown in blue, and the differences in hot-evolved lineages and acclimated controls is shown in red. Data are averaged for all 6 replicates across the last year of the experiment (2-3 time points) with error bars showing +/- 1 standard error. We measured growth rate (panel A), cellular C content (panel B), chlorophyll (panel C), photosynthesis (panel D), attack rate (panel E), and respiration (panel F). Significant differences are shown by each bar (one-sample Student’s t-test; *** = p<0.001, ** = p<0.01, and * = p<0.05).
...
#growth
del.gr.18 <- growths100_last3.91 %>% 
  filter(Ev == 18) %>% 
  pull(Growth.del) %>% 
  mean
del.gr.30 <- growths100_last3.91 %>% 
  filter(Ev == 30) %>% 
  pull(Growth.del) %>% 
  mean
...
del.redf.18 <- growths100_last3.91 %>% 
  filter(Ev == 18) %>% 
  pull(redf.del) %>% 
  mean
del.redf.30 <- growths100_last3.91 %>% 
  filter(Ev == 30) %>% 
  pull(redf.del) %>% 
  mean
...
#attack
del.attack.18 <- attack100_last3.91 %>% 
  filter(Ev == 18) %>% 
  pull(Attack.del) %>% 
  mean
del.attack.30 <- attack100_last3.91 %>% 
  filter(Ev == 30) %>% 
  pull(Attack.del) %>% 
  mean
val.attack.30 <- attack100_last3 %>% 
  filter(Ev == 24 & Acc == 30) %>% 
  filter(Strain == 1391) %>% 
  pull(attack) %>% 
  mean
val.attack.18 <- attack100_last3 %>% 
  filter(Ev == 24 & Acc == 18) %>% 
  filter(Strain == 1391) %>% 
  pull(attack) %>% 
  mean
...
#Resp/photo
del.resp.18 <- resp.100_last3.91 %>% 
  filter(Ev == 18) %>% 
  pull(Resp.del) %>% 
  mean
del.resp.30 <- resp.100_last3.91 %>% 
  filter(Ev == 30) %>% 
  pull(Resp.del) %>% 
  mean
...
del.photo.18 <- resp.100_last3.91 %>% 
  filter(Ev == 18) %>% 
  pull(Photo.del) %>% 
  mean
del.photo.30 <- resp.100_last3.91 %>% 
  filter(Ev == 30) %>% 
  pull(Photo.del) %>% 
  mean
...
hotattack <- abs(del.attack.30  /val.attack.30) * 100
...
pe <- read_csv(here::here("data","pestats_trc_rev3.csv")) 
graz <- read_csv(here::here("data","TRC_grazstats_dilution_rev3.csv")) 

# grazing pgC per och per day (bact per och per day * C per bact * fg to pgC * 24hrs)
graz$graz.day <- graz$G_O *24 * 113.5996283 * 0.001 
#new
graz$g.alpha.day <- graz$a.bestfit *24  
graz$g.alpha.day.2 <-graz$a.bestfit *24 *10^5
...
#### copy notes from notebook on how you got is conversion!!!
...
#	plot	the	predictions
  fig5a<- ggplot(tmp18_pi3,	aes(x	=	temperature,	y	=	mu))	+ 
    ...
    scale_x_continuous("")	+ scale_y_continuous("Electron transport rate e-1 pgC-1 day-1 (x10^15)")	+
...
# geom smooth using loess and default span of .75
 fig5b<-  ggplot(pe, aes(x = Assay, y = Fv.Fm)) +
...
theme(legend.position = "none",
        ...
        axis.text.x=element_text(size = 11),
        axis.ticks.length = unit(.1, "cm")) +
  labs(y = "Photosynthetic efficiency Fv/Fm")
...
fig5<-ggarrange(fig5a, fig5b,
                ...
                widths = 1,
                heights = 1)
  annotate_figure(fig5, bottom = "Assay temperature C")

  
  
```

Figure 5. Thermal response curves of two photosynthetic traits for obligate phototroph at high light. Electron transport rate per pgC (panel A) and photosynthetic efficiency (panel B) were measured at 10 temperatures between 3°C and 44°C for cold-evolved (blue), control (gray), and hot-evolved (red) lineages at the termination of the evolution experiment. For the cold-evolved lineages this represents >200 generations, and for the hot-evolved lineages, this represents >600 generations. Curves with confidence intervals represent the average of all 6 lineages at each temperature and individual lineages are represented by points. Three vertical lines farthest to the left represent the thermal optimum of each temperature, while the vertical lines on the right represent the thermal maxima. 
  ...
  geom_point(aes(color = as.factor(Ev)), alpha = .5, size = 1.5) +
   ...
   geom_smooth(aes(x = Assay, y = graz.C.day, color = as.factor(Ev),fill = as.factor(Ev)),alpha = .4, method = "loess", span = .75, size = .5) +
     ...
     theme(legend.position = "none",
        ...
        axis.text.x=element_text(size = 11),
        axis.ticks.length = unit(.1, "cm")) +
  labs(x = "Assay temperature C", y = "Grazing gC gC-1 day-1")
...
Figure S5. Thermal response curves of grazing rates for obligate phototroph at high light. Grazing rate was measured at 10 temperatures between 18°C and 39°C for cold-evolved (blue), control (gray), and hot-evolved (red) lineages at the termination of the evolution experiment. For the cold-evolved lineages this represents >200 generations, and for the hot-evolved lineages, this represents >600 generations. Curves with confidence intervals represent the average of all 6 lineages at each temperature and individual lineages are represented by points. Three vertical lines farthest to the left represent the thermal optimum of each temperature, while the vertical lines on the right represent the thermal maxima. 


### Supporting Information


Figure S3 all growth rates from back transfer experiments 
...
Growth.summ.100.1391.time <- Growth.summ.100.time %>% 
  filter(Strain == 1391)
Growth.summ.100.2951.time <- Growth.summ.100.time %>% 
  filter(Strain == 2951)
redf.corr.summ.100.1391.time <- redf.corr.summ.100.time %>% 
  filter(Strain == 1391)
...
Growth.summ.50.1391.time <- Growth.summ.50.time %>% 
  filter(Strain == 1391)
Growth.summ.50.2951.time <- Growth.summ.50.time %>% 
  filter(Strain == 2951)
redf.corr.summ.50.1391.time <- redf.corr.summ.50.time %>% 
  filter(Strain == 1391)
...
s1.1<- ggplot(Growth.summ.100.1391.time, 
       aes(x=Acc, y=Growth.Rate, color=as.factor(Ev))) + 
...
s1.2<- ggplot(Growth.summ.50.1391.time, 
       aes(x=Acc, y=Growth.Rate, color=as.factor(Ev))) + 
    geom_errorbar(aes(ymin=Growth.Rate-se, ymax=Growth.Rate+se), width=1) +
    geom_line(lwd = 1) +
    geom_point(size = 3)+
...
# order 1391 100, 1391 50, 2951 100, 2951 50

figs3<-ggarrange(s1.1, s1.2, s1.3,s1.4,
   labels = c("A", "B", "C","D"),
   font.label = list(size = 12, color = "black", family = NULL),
          label.x = 0,
          label.y = 1,
          ncol = 1, nrow = 4,align="hv" )
annotate_figure(figs3, bottom = "Temperature C", top = "Experimental week", left = "Growth rate d-1")
...
```
Figure S3. Thermal reaction norms of growth rate for all lineages at every reciprocal transplant time point. Panel A shows the obligate phototroph at high light and panel B at low light. Panel C is the facultative phototroph at high light and panel D at low light. Each box is a single time point labeled as experimental week going from left to right, with the x-axis of each box showing temperature. Blue points and lines are cold-evolved linages, red points and lines are hot-evolved lineages, and gray points and lines are control lineages. All points are averages of 6 replicates with bars depicting standard error. 


Figure S2. QAQC showing correlation from extracted chl-a to redf

```{r}
qaqc<-read_csv(here::here("data","QAQC_data_Jan2021 - Export data_rev3.csv"))
...
legend(3000,.5,pch=21,pt.bg=c('gray80','black'),legend=c('1391','2951'))

lm.redF.91 <- lm(ChlperCell~RedF,data=qaqc[qaqc$Strain==1391,])
summary(lm.redF.91)
...
lm_eqn91 <- function(lm.91){
    m <- lm(ChlperCell ~ RedF, lm.91);
    eq <- substitute("Strain 1391:"~~italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
...
ggplot(qaqc, aes(x = RedF, y = ChlperCell, fill = as.factor(Strain)))+
  ...
  geom_abline(intercept = coefficients(lm.redF.91)[1], slope = coefficients(lm.redF.91)[2], color = "gray80") +
...
labs( y ="Chlorophyll per cell", x="Red fluorescence") +
   geom_text(x = 3000, y = .5, label = lm_eqn91(lm.91), parse = TRUE, color = "gray50", hjust = "left", size = 3.5)+
  geom_text(x = 3000, y = .45, label = lm_eqn51(lm.51), parse = TRUE, hjust= "left", size = 3.5)

```

Figure S2. Correlation between extracted chlorophyll per cell measurements from a fluorometer and red fluorescence from flow cytometer for each strain of Ochromonas (obligate phototroph Strain 1391 in gray, and facultative phototroph Strain 2951 in black). 
